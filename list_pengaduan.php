...
if ($_POST['setuju'])
{
	...
	{
	$data = $_POST["id"][$i];
	$ubah_status = "UPDATE laporan_keluhan SET status = 1 where id_laporan = $data";
...
Status</TH><TH>Proses</TH></TR></thead><?php
	...
	foreach($listpengaduan as $pengaduan){ 
	 ?>
		<tbody>
        <TR>
        <TD><?php echo"<input type=checkbox name=kotak[] value=". $pengaduan->id_laporan ." id=$no>"; $no++; ?></TD>
        <TD><?php 
		if($pengaduan->status==1)
		{
			$status="Disetujui";$url="Terkirim";
		}
		elseif($pengaduan->status==2)
		{
			$status="Dihapus";$url="Terhapus";
		}else{ 
			$status="Ditangguhkan" ;$url="<a href='?page=pengaduan&id_laporan_setuju=".$pengaduan->id_laporan."'>Setuju</a>";
			} echo $pengaduan->id_laporan  . "</TD>
