...
<?php
	...
	{
		$select_all="SELECT COUNT(*) as num FROM fix_vw_t_surat_keterangan_riwayat_tanah where c_type_form = 14";
	}
	

	global $wpdb;
	$tableName="fix_vw_t_surat_keterangan_riwayat_tanah";		
	$targetpage = "?page=sk_riwayat_tanah"; 	
	echo get_permalink();
	$limit = 20; 
	
	$total_pages = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) as num FROM fix_vw_t_surat_keterangan_riwayat_tanah where c_type_form = 14 "));
    ...
    // Get page 
	$data_pages="select * from fix_vw_t_surat_keterangan_riwayat_tanah LIMIT $start, $limit";
	//echo $data_pages;
    $listpengaduan=$wpdb->get_results($wpdb->prepare($data_pages));
...
echo '<meta http-equiv="refresh" content="0; url=?page=sk_riwayat_tanah&paging='.$paging.' ">';
}
...
// pagination
 echo $paginate;
	...
	{
		include"form_edit/sk_riwayat_tanah.php";
	}
	?>
    ...
    <form action="" method="post" name="myform">
        ...
        <thead>
			...
			<?php
            $no=0;
            foreach($listpengaduan as $pengaduan){ $no++
            ?>
                <tbody>
                    <tr>
                        ...
                        <td>
							...
							{
								$url="<a href='?page=sk_riwayat_tanah&id_laporan_setuju=".$pengaduan->id."&status_id_next=".$pengaduan->c_status_next."&id_warga=".$pengaduan->id_warga."&page_form=".$pengaduan->c_type_form."'>Lanjut</a>";
							}
							...
							foreach($list_cek_if_print as $cek_if_print)
							{
								...
								{
									$url_edit=" | <a href='?page=sk_riwayat_tanah&id_laporan_sunting=".$pengaduan->id."'>Sunting</a>";
								}
							}						
							echo $pengaduan->id  . "</td>
                            <td>" . $pengaduan->c_nama . "</td>
                            <td>" . $pengaduan->c_no_waris . "</td>
                            <td>" . $status . "  " . "</td>
...
function getConfirmation(){
   ...
   if( retVal == true ){
		...
		foreach($list_cek_tiga_rebu_sec as $cek_tiga_rebu_sec)
		{
			...
			{
		?>
			window.open("<?php echo get_bloginfo('url'); ?>/export-pdf/pondok_kelapa/laporan_pondok_kelapa_sk_riwayat_tanah.php?id_cetak_surat=<?php echo $_GET['id_laporan_setuju'];?>");
		<?php
			}
		}
		?>
