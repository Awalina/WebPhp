<?php
	...
	global $wpdb;
	$tableName="fix_vw_t_surat_keterangan_kem_suami_istri";
	$targetpage = "?page=sk_surat_keterangan_kem_suami_istri";  	
	echo get_permalink();
	$limit = 20;  
...
echo '<meta http-equiv="refresh" content="0; url=?page=sk_surat_keterangan_kem_suami_istri&paging='.$paging.' ">';
}
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
							...
							{
								$url="<a href='?page=sk_kematian_suami_istri&id_laporan_setuju=".$pengaduan->id."&status_id_next=".$pengaduan->c_status_next."&id_warga=".$pengaduan->id_warga."&page_form=".$pengaduan->c_type_form."'>Lanjut</a>";
...
function getConfirmation(){
   ...
   if( retVal == true ){
		...
		foreach($list_cek_tiga_rebu_sec as $cek_tiga_rebu_sec)
		{
			...
			{
		?>
			window.open("<?php echo get_bloginfo('url'); ?>/export-pdf/pondok_kelapa/laporan_pondok_kelapa_sk_kematian_suami_istri.php?id_cetak_surat=<?php echo $_GET['id_laporan_setuju'];?>");
		<?php
			}
		}
		?>
...
<?php
	...
	if(isset($_REQUEST['submit_reason']))
	{
		...
		if ($test) 
		{
			...
			echo '<meta http-equiv="refresh" content="0; url=?page=sk_kematian_suami_istri&paging='.$paging.' ">';
		}
