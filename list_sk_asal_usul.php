<?php
	...
	global $wpdb;
	$tableName="fix_vw_surat_keterangan_asal_usul_first";		
	$targetpage = "?page=sk_asal_usul"; 	
	echo get_permalink();
	$limit = 20; 
...
echo '<meta http-equiv="refresh" content="0; url=?page=sk_asal_usul&paging='.$paging.' ">';
}
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
							...
							{
								$url="<a href='?page=sk_asal_usul&id_laporan_setuju=".$pengaduan->id."&status_id_next=".$pengaduan->c_status_next."&id_warga=".$pengaduan->id_warga."&page_form=".$pengaduan->c_surat."'>Lanjut</a>";
...
function getConfirmation(){
   ...
   if( retVal == true ){
		...
		foreach($list_cek_tiga_rebu_sec as $cek_tiga_rebu_sec)
		{
			...
			{
		?>
			window.open("<?php echo get_bloginfo('url'); ?>/export-pdf/pondok_kelapa/laporan_pondok_kelapa_sk_asal_usul.php?id_cetak_surat=<?php echo $_GET['id_laporan_setuju'];?>");
		<?php
			}
		}
		?>
