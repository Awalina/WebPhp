<?php
	...
	{
		$select_all="SELECT COUNT(*) as num FROM $table";
	}
	

	global $wpdb;
	$tableName="fix_vw_surat_keterangan_first";		
	$targetpage = "?page=sk_keterangan_waris"; 	
	echo get_permalink();
	$limit = 20; 
	
	$total_pages = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) as num FROM fix_vw_surat_keterangan_first where c_type_form = 10"));
    ...
    // Get page data
    $listpengaduan=$wpdb->get_results($wpdb->prepare("select *from fix_vw_surat_keterangan_first where c_type_form = 10 LIMIT $start, $limit"));
...
echo '<meta http-equiv="refresh" content="0; url=?page=sk_keterangan_waris&paging='.$paging.' ">';
}
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            ...
                            {
                            $url="<a href='?page=sk_keterangan_waris&id_laporan_setuju=".$pengaduan->id."'>Selesai</a> | <a href='?page=sk_keterangan_waris&id_laporan_tidak_setuju=".$pengaduan->id."'>Tidak Selesai</a>";
                            }else if($pengaduan->status_id==2){
								$url="";
								}else{
								$url="<a href='".get_bloginfo('url')."/export-pdf/pondok_kelapa/laporan_pondok_kelapa_sk_keterangan_waris.php?id_cetak_surat=".$pengaduan->id."' target='_Blank'>Cetak Surat</a>";
...
<?php
	...
	{
		$test=$wpdb->query($wpdb->prepare("UPDATE  t_surat_keterangan SET status_id = 1 WHERE id = ". $_REQUEST['id_laporan_setuju'].""));
	...
	{
		$test=$wpdb->query($wpdb->prepare("UPDATE  t_surat_keterangan SET status_id = 2,reason='".$_POST['reason']."' WHERE id = ". $_REQUEST['id_surat_batal'].""));
		//echo '<meta http-equiv="refresh" content="0; url=?page=surat_keterangan&paging='.$paging.' ">';
		if ($test) 
		{
			echo "<script language='javascript'>alert('Data berhasil disimpan');</script>";
