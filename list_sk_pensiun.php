<?php
	...
	{
		$select_all="SELECT COUNT(*) as num FROM fix_vw_surat_keterangan_first where c_type_form = 9";
	}
	

	global $wpdb;
	$tableName="fix_vw_surat_keterangan_first";		
	$targetpage = "?page=sk_pensiun"; 	
	echo get_permalink();
	$limit = 20; 
	
	$total_pages = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) as num FROM fix_vw_surat_keterangan_first where c_type_form = 9"));
    ...
    // Get page data
    $listpengaduan=$wpdb->get_results($wpdb->prepare("select *  from fix_vw_surat_keterangan_first where c_type_form =9 LIMIT $start, $limit"));
...
echo '<meta http-equiv="refresh" content="0; url=?page=sk_pensiun&paging='.$paging.' ">';
}
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
							...
							{
								$url="<a href='?page=sk_pensiun&id_laporan_setuju=".$pengaduan->id."&status_id_next=".$pengaduan->c_status_next."&id_warga=".$pengaduan->id_warga."&page_form=".$pengaduan->c_type_form."'>Lanjut</a>";
...
function getConfirmation(){
   ...
   if( retVal == true ){
		...
		foreach($list_cek_tiga_rebu_sec as $cek_tiga_rebu_sec)
		{
			...
			{
		?>
			window.open("<?php echo get_bloginfo('url'); ?>/export-pdf/pondok_kelapa/laporan_pondok_kelapa_sk_pensiun.php?id_cetak_surat=<?php echo $_GET['id_laporan_setuju'];?>");
		<?php
			}
		}
		?>
