<?php
	...
	{
		$select_all="SELECT COUNT(*) as num FROM t_surat_keterangan where c_type_form = 16";
	}
	

	global $wpdb;
	$tableName="t_surat_keterangan";		
	$targetpage = "?page=1955"; 	
	echo get_permalink();
	$limit = 20; 
	
	$total_pages = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) as num FROM t_surat_keterangan where c_type_form = 16"));
    ...
    // Get page data
    $listpengaduan=$wpdb->get_results($wpdb->prepare("select * from t_surat_keterangan where c_type_form = 16 LIMIT $start, $limit"));
...
echo '<meta http-equiv="refresh" content="0; url=?page_id=1955&paging='.$paging.' ">';
}
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
							...
							{
							$url="<a href='?page=1955&id_laporan_setuju=".$pengaduan->id."&status_id_next=".$pengaduan->c_status_next."&id_warga=".$pengaduan->id_warga."&page_form=".$pengaduan->c_type_form."'>Lanjut</a>";
							}
							...
							foreach($list_cek_if_print as $cek_if_print)
							{
								if($pengaduan->status_id >= $cek_if_print->value)
								{
									...
									$url_edit=" | <a href='?page_id=1955&id_laporan_sunting=".$pengaduan->id."'>Sunting</a>";
								}
							}
							echo $pengaduan->id  . "</td>
                            <td>" . $pengaduan->c_no_ktp . "</td>
                            <td>" . $pengaduan->c_nama . "</td>
                            <td>" . $pengaduan->c_keperluan . "</td>
                            <td>" . $status . "  " . "</td>
...
{
?>
window.open("<?php echo get_bloginfo('url'); ?>/export-pdf/pondok_kelapa/laporan_pondok_kelapa_sk_persyaratan_lain_lain.php?id_cetak_surat=<?php echo $_GET['id_laporan_setuju'];?>");
<?php
}
}
?>
...
<?php
	...
	{
		$test=$wpdb->query($wpdb->prepare("insert into t_transaksi_all_surat (`id_warga`, `id_surat`, `status_id`, `keterangan`) values ('".$_POST['id_warga_post']."','".$_POST['page_form_post']."','".$_POST['status_id_next_post']."','".$_POST['reason']."')"));//echo '<meta http-equiv="refresh" content="0; url=?page=surat_keterangan&paging='.$paging.' ">';
		if ($test) 
		{
			echo "<script language='javascript'>alert('Data berhasil disimpan');</script>";
			echo '<meta http-equiv="refresh" content="0; url=?page=sk_persyaratan_lain_lain&paging='.$paging.' ">';
		}
