<?php
	...
	{
		$select_all="SELECT COUNT(*) as num FROM fix_vw_surat_keterangan_first where c_type_form = 23";
	}
	

	global $wpdb;
	$tableName="fix_vw_surat_keterangan_first";		
	$targetpage = "?page=sk_izin_pesta"; 	
	echo get_permalink();
	$limit = 20; 
	
	$total_pages = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) as num FROM fix_vw_surat_keterangan_first where c_type_form = 23"));
    ...
    // Get page data
    $listpengaduan=$wpdb->get_results($wpdb->prepare("select * from fix_vw_surat_keterangan_first where c_type_form = 23 LIMIT $start, $limit"));
...
echo '<meta http-equiv="refresh" content="0; url=?page=sk_izin_pesta&paging='.$paging.' ">';
}
...
?>
	...
	{
		include"form_edit/sp_izin_pesta.php";
	}
	?>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
							...
							{
							$url="<a href='?page=sk_izin_pesta&id_laporan_setuju=".$pengaduan->id."&status_id_next=".$pengaduan->c_status_next."&id_warga=".$pengaduan->id_warga."&page_form=".$pengaduan->c_type_form."'>Lanjut</a>";
							}
							...
							foreach($list_cek_if_print as $cek_if_print)
							{
								if($pengaduan->status_id >= $cek_if_print->value)
								{
									...
									$url_edit=" | <a href='?page=sk_izin_pesta&id_laporan_sunting=".$pengaduan->id."'>Sunting</a>";
								}
							}
...
{
?>
window.open("<?php echo get_bloginfo('url'); ?>/export-pdf/pondok_kelapa/laporan_pondok_kelapa_sk_izin_pesta.php?id_cetak_surat=<?php echo $_GET['id_laporan_setuju'];?>");
<?php
}
}
?>
