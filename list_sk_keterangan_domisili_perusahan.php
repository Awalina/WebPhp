<?php
	...
	{
		$select_all="SELECT COUNT(*) as num FROM fix_vw_surat_domisili_perusahaan_first ";
	}
	
	global $wpdb;
	$tableName="fix_vw_surat_domisili_perusahaan_first";		
	$targetpage = "?page=sk_keterangan_domisili_perusahaan"; 	
	echo get_permalink();
	$limit = 20; 
	
	$total_pages = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) as num FROM fix_vw_surat_domisili_perusahaan_first"));
    ...
    // Get page data
    $listpengaduan=$wpdb->get_results($wpdb->prepare("select *  from fix_vw_surat_domisili_perusahaan_first LIMIT $start, $limit "));
...
if ($_POST['delete'])
{
	...
	{
	$data = $_POST["id"][$i];
	$delete = "delete from t_surat_domisili_perusahaan where id = $data";
...
echo '<meta http-equiv="refresh" content="0; url=?page=sk_keterangan_domisili_perusahaan&paging='.$paging.' ">';
}
if ($_POST['setuju'])
{
	$_POST["id"] = $_POST['kotak'];
	...
	{
	$data = $_POST["id"][$i];
	$ubah_status = "UPDATE t_surat_domisili_perusahaan SET status_id = 1 where id = $data";
...
<?php
	...
	{
		include"form_edit/sk_keterangan_domisili_perusahaan.php";
	}
	?>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
							...
							{
							$url="<a href='?page=sk_keterangan_domisili_perusahaan&id_laporan_setuju=".$pengaduan->id."&status_id_next=".$pengaduan->c_status_next."&id_warga=".$pengaduan->id_warga."&page_form=".$pengaduan->c_type_form."'>Lanjut</a>";
							}
							...
							foreach($list_cek_if_print as $cek_if_print)
							{
								if($pengaduan->status_id >= $cek_if_print->value)
								{
									...
									$url_edit=" | <a href='?page=sk_keterangan_domisili_perusahaan&id_laporan_sunting=".$pengaduan->id."'>Sunting</a>";
								}
							}
							echo $pengaduan->id  . "</td>
                            <td>" . $pengaduan->c_nama_pendaftar . "</td>
...
function getConfirmation(){
   ...
   if( retVal == true ){
		...
		foreach($list_cek_tiga_rebu_sec as $cek_tiga_rebu_sec)
		{
			...
			{
		?>
			window.open("<?php echo get_bloginfo('url'); ?>/export-pdf/pondok_kelapa/laporan_pondok_kelapa_sk_domisili_perusahaan.php?id_cetak_surat=<?php echo $_GET['id_laporan_setuju'];?>");
		<?php
			}
		}
		?>
...
<?php
	...
	if($_POST['delete'])
	{
		...
		$no=mysql_query("delete from t_surat_domisili_perusahaan where id_laporan='$cek[$i]'");
