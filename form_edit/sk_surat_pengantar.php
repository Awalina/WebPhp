...
<?php
$list_sel_get_sunting=$wpdb->get_results($wpdb->prepare("select * from fix_vw_t_surat_pengantar where id='$_GET[id_laporan_sunting]'"));
...
<input name="ktp" id="ktp" size="55" value="<?php echo $sel_get_sunting->c_nik; ?>" />           
</td>
</tr>
<tr>
...
{	
	...
	$insert_db= mysql_query("update t_surat_pengantar set c_nama='$nama',c_tempat_lahir='$temp_lahir',c_tgl_lahir='$tgl_lahir', c_jenis_kelamin='$jenis_kelamin', ref_agama='$agama', c_pekerjaan='$pekerjaan', c_nik='$ktp', c_alamat='$alamat', c_keperluan='$keperluan', id_warga='$no_rt_rw' where id='$id_surat_laporan'");	
		if($insert_db)
		{
			echo"<script>alert('Proses Sunting Berhasil');location.href='?page=surat_pengantar'</script>";
		}
		else
		{
