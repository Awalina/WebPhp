<?php
	...
	{
		$select_all="SELECT COUNT(*) as num FROM fix_vw_t_surat_ket_ortu_first ";
	}
	

	global $wpdb;
	$tableName="fix_vw_t_surat_ket_ortu_first";		
	$targetpage = "?page=sk_tentang_orangtua"; 	
	echo get_permalink();
	$limit = 20; 
	
	$total_pages = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) as num FROM fix_vw_t_surat_ket_ortu_first "));
    ...
    // Get page data
    $listpengaduan=$wpdb->get_results($wpdb->prepare("select * from fix_vw_t_surat_ket_ortu_first LIMIT $start, $limit "));
...
echo '<meta http-equiv="refresh" content="0; url=?page=sk_tentang_orangtua&paging='.$paging.' ">';
}
...
<?php
	...
	{
		include"form_edit/sk_tentang_orangtua.php";
	}
	?>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
							...
							{
								$url="<a href='?page=sk_tentang_orangtua&id_laporan_setuju=".$pengaduan->id."&status_id_next=".$pengaduan->c_status_next."&id_warga=".$pengaduan->id_warga."&page_form=".$pengaduan->c_surat."'>Lanjut</a>";
							}
							...
							foreach($list_cek_if_print as $cek_if_print)
							{
								if($pengaduan->status_id >= $cek_if_print->value)
								{
									...
									$url_edit=" | <a href='?page=sk_tentang_orangtua&id_laporan_sunting=".$pengaduan->id."'>Sunting</a>";
								}
							}
							echo $pengaduan->id  . "</td>
                            <td>" . $pengaduan->c_nama_ayah . "</td>
                            <td>" . $pengaduan->c_nama_ibu . "</td>
                            <td> Membuat " . $pengaduan->c_nama_surat . "</td>
                            <td>" . $status . "  " . "</td>
...
function getConfirmation(){
   ...
   if( retVal == true ){
		...
		foreach($list_cek_tiga_rebu_sec as $cek_tiga_rebu_sec)
		{
			...
			{
		?>
			window.open("<?php echo get_bloginfo('url'); ?>/export-pdf/pondok_kelapa/laporan_pondok_kelapa_sk_tentang_orang_tua.php?id_cetak_surat=<?php echo $_GET['id_laporan_setuju'];?>");
		<?php
			}
		}
		?>
